...
def main():
    print("="*70)
    print("🚀 启动 2025 年 1-12 月 2D 空间连续反演与 AE 绝对误差全局组图管线")
    print("="*70)
    ...
    V_ERR_AE = 20               # AE 绝对误差范围 
    cmap_ae = plt.cm.RdBu_r     # 发散色标：红高估，蓝低估
    ...
    for idx, target_month in enumerate(TARGET_MONTHS):
        ...
        if not test_month_df.empty:
            ...
            if not valid_grid.empty:
                ...
                # 🌟 计算平均误差 ME = Pred - Obs
                site_monthly_stats['ME'] = site_monthly_stats['pred_mean'] - site_monthly_stats['obs_mean']
                site_monthly_stats = site_monthly_stats.dropna(subset=['ME'])
        ...
        if not site_monthly_stats.empty:
            scatter_err = ax.scatter(site_monthly_stats['lon'].values, site_monthly_stats['lat'].values, 
                                     c=site_monthly_stats['ME'].values, 
                                     cmap=cmap_ae, vmin=-V_ERR_AE, vmax=V_ERR_AE, 
    ...
    if sm_err is not None:
        ...
        cbar2.set_label(r'【圆圈站点】独立测试站点 ME 平均误差 ($\mu g/m^3$)', fontproperties=my_font, fontsize=24, weight='bold')
    ...
    save_path = os.path.join(FIG_DIR, "Spatial_Monthly_ME_2025_12Months.png")
    plt.savefig(save_path, bbox_inches='tight')
    plt.close()
    
    print("="*70)
    print(f"🎉 恭喜！12个月份的 ME 平均误差全矩阵大组图已生成！\n👉 图片保存至: {save_path}")
    print("="*70)
