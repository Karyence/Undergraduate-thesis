...
OUTPUT_FILE = os.path.expanduser("~/bisheshuju/ERA5_LAND/ERA5_d2m_1km_downscaled.nc")
...
def calc_detailed_stats(da):
    ...
    return {
        ...
        "经度最小值(°)": f"{lon_vals.min():.3f}",
        "经度最大值(°)": f"{lon_vals.max():.3f}",
        "经度网格点数": f"{len(lon_vals)}",
        "纬度最小值(°)": f"{lat_vals.min():.3f}",
        "纬度最大值(°)": f"{lat_vals.max():.3f}",
        "纬度网格点数": f"{len(lat_vals)}",
        "时间序列长度(小时)": f"{da.shape[0]}",  
        "露点温度最小值(℃)": f"{float(da.min().compute()):.2f}",
        "露点温度最大值(℃)": f"{float(da.max().compute()):.2f}"
    }

def print_final_comparison(stats_before, stats_after):
    """打印降尺度前后数据对比报告"""
    print("\n" + "="*90)
    print("📊 ERA5 d2m (2m露点温度) 降尺度前后详细数据对比报告".center(86))
    print("="*90)
...
def process_d2m():
    ...
    print(f"🚀 已启动集群，监控地址: {client.dashboard_link}")

    print("🚀 开始执行 ERA5 2m露点温度 (d2m) 小时级降尺度")
    ...
    datasets = []
    drop_vars = ['t2m', 'skt', 'sp', 'tp', 'u10', 'v10'] 
    ...
    # 3. 单位转换 
    print("[处理中] 执行单位转换并提取 2025 全年数据...")
    d2m = ds_concat['d2m'] - 273.15 
    # 直接切片
    d2m_hourly = d2m.sel(time=slice("2025-01-01", "2025-12-31"))

    # 获取插值前的统计特征 
    stats_before = calc_detailed_stats(d2m_hourly)

    # 4. 执行双线性插值
    print("[处理中] 映射至 1km 网格并执行空间插值 (延迟计算)...")
    d2m_interp = d2m_hourly.interp(
    ...
    # 对比分析
    stats_after = calc_detailed_stats(d2m_interp)
    ...
    d2m_interp.attrs = {
        ...
        'description': 'Hourly mean, converted from K to C, bilinearly downscaled to 1km grid.'
    }
    ...
    d2m_interp.to_netcdf(OUTPUT_FILE, encoding=encoding, engine='netcdf4', compute=True)
