...
def generate_academic_figures(agg_df_daily, comp_df_hourly, output_dir):
    ...
    def plot_density_scatter(ax, y_t, y_p, title):
        ...
        cb.set_label('Data Point Density (Count)', fontsize=13, weight='bold')
        ...
        # 方案A：子图标题仅保留 (a) 和 (b) 标号
        ax.set_title(title, loc='left', fontsize=16, weight='bold', pad=15)
        ax.set_xlabel(r'Observed PM$_{\mathbf{2.5}}$ Concentration ($\boldsymbol{\mu}\mathbf{g}/\mathbf{m}^{\mathbf{3}}$)', fontsize=14, weight='bold')
        ax.set_ylabel(r'Predicted PM$_{\mathbf{2.5}}$ Concentration ($\boldsymbol{\mu}\mathbf{g}/\mathbf{m}^{\mathbf{3}}$)', fontsize=14, weight='bold')
    ...
    plt.close()
    print(f"  ✅ 对数密度散点图 已生成: {scatter_path}")
    ...
    ax1 = axes[0]
    bars1_agg = ax1.bar(x - width/2, rmse_agg_s, width, label='Hourly Training & Daily Aggregation', color='#1f77b4', edgecolor='black', alpha=0.85)
    bars1_dir = ax1.bar(x + width/2, rmse_dir_s, width, label='Direct Daily Training', color='#ff7f0e', edgecolor='black', alpha=0.85)
    ax1.set_ylabel(r'Absolute RMSE ($\boldsymbol{\mu}\mathbf{g}/\mathbf{m}^{\mathbf{3}}$)', fontsize=15, weight='bold')
    ax1.set_title('(a)', loc='left', fontsize=17, weight='bold', pad=15)
    ax1.set_xticks(x)
    ax1.set_xticklabels(season_names, fontsize=14, weight='bold')
    ax1.grid(axis='y', linestyle='--', alpha=0.5)
    ax1.set_ylim(0, max(max(rmse_agg_s), max(rmse_dir_s)) * 1.2)
    ax1.tick_params(axis='y', labelsize=12)
    
    ax1_twin = ax1.twinx()
    ax1_twin.plot(x, rmse_diff_s, color='red', marker='D', markersize=8, linewidth=2.5, label=r'Accuracy Gain ($\Delta$RMSE)', zorder=10)
    ax1_twin.set_ylabel(r'Accuracy Gain $\Delta$RMSE ($\boldsymbol{\mu}\mathbf{g}/\mathbf{m}^{\mathbf{3}}$)', fontsize=15, color='red', weight='bold')
    ...
    ax1.legend(lines_1 + lines_2, labels_1 + labels_2, loc='upper center', prop={'size':11}, frameon=True)

    # === 子图 (b): 污染等级对比 ===
    bins = [0, 35, 75, 1000]
    labels_level = ['Good\n(<35)', 'Light Pollution\n(35-75)', 'Moderate-Heavy Pollution\n($\geq$75)']
    comp_df_hourly['Level'] = pd.cut(comp_df_hourly['pm25_hourly'], bins=bins, labels=labels_level, right=False)
    
    rmse_agg_l, rmse_dir_l, rmse_diff_l = [], [], []
    for lvl in labels_level:
    ...
    x_l = np.arange(len(labels_level))
    
    ax2 = axes[1]
    bars2_agg = ax2.bar(x_l - width/2, rmse_agg_l, width, label='Hourly Training & Daily Aggregation', color='#1f77b4', edgecolor='black', alpha=0.85)
    bars2_dir = ax2.bar(x_l + width/2, rmse_dir_l, width, label='Direct Daily Training', color='#ff7f0e', edgecolor='black', alpha=0.85) 
    ax2.set_ylabel(r'Absolute RMSE ($\boldsymbol{\mu}\mathbf{g}/\mathbf{m}^{\mathbf{3}}$)', fontsize=15, weight='bold')
    ax2.set_title('(b)', loc='left', fontsize=17, weight='bold', pad=15)
    ax2.set_xticks(x_l)
    ax2.set_xticklabels(labels_level, fontsize=13, weight='bold')
    ax2.grid(axis='y', linestyle='--', alpha=0.5)
    ax2.set_ylim(0, max(max(rmse_agg_l), max(rmse_dir_l)) * 1.2)
    ax2.tick_params(axis='y', labelsize=12)
    
    ax2_twin = ax2.twinx()
    ax2_twin.plot(x_l, rmse_diff_l, color='red', marker='D', markersize=8, linewidth=2.5, label=r'Accuracy Gain ($\Delta$RMSE)', zorder=10)
    ax2_twin.set_ylabel(r'Accuracy Gain $\Delta$RMSE ($\boldsymbol{\mu}\mathbf{g}/\mathbf{m}^{\mathbf{3}}$)', fontsize=15, color='red', weight='bold')
